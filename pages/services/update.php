...
if($_POST && Security::isVerified(['update', 'nom'], Security::POST_TYPE)) {
    ...
    $update = ServiceController::update( Security::secure($_GET['id']), $post_data);
...
<?php else : ?>
    <div class="bg-white mt-4 p-4 border-0 border-top border-5 border-success rounded mx-auto" style="max-width:600px">
        <h2 class="text-success my-4">Modification du service</h2>
        <form action="<?= Security::secure($_SERVER['PHP_SELF']) . '?id='. $id ?> " class="form" method="POST">
            <div class="form-group mb-3">
                <input type="text" name="nom" value="<?= isset($post_data['nom']) ? $post_data['nom'] : Security::secure($service->nom) ?>" id="nom" class="form-control" placeholder="Votre nom">
